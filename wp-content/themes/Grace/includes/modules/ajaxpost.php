...
function fa_make_post_section(){
    ...
    if( $dis_author == 1 ){
        if( $cc_value != 2 ){
            $post_html .= '<li class="post-author"><div class="avatar">'. get_avatar( get_the_author_meta('ID') ).'</div><a href="'. get_author_posts_url( get_the_author_meta( 'ID' ) ). '" target="_blank">'. get_the_author() .'</a></li>'; 
       } else {
            $post_html .= '<li class="post-author"><div class="avatar">'. cc_avatar().'</div>'.get_post_meta($post->ID,'cus_author_name',true).'</li>';
       }
        
    }
    if( $cc_value == 1 ){
        $post_html .= '<li class="postoriginal"><span><i class="icon-cc-1"></i>'.suxingme('suxingme_custom_cc').'</span></li>';
    }
    if( $dis_cat == 1 ){
    ...
    if( $dis_view == 1 ){
        $post_html .= '<li class="ico-eye"><i class="icon-eye-4"></i> '.post_views('','',0).'</li>';
    }
    if( $dis_like == 1 ){
        $post_html .= '<li class="ico-like"><i class="icon-heart"></i> '.$ding_num.'</li>';
    }
    ...
    if( has_post_format( 'gallery' ) ){
        ...
        $post_section .= '<h2>'.suxing_post_state_date().'<a href="' . get_permalink() . '" title="' . get_the_title() . '" '.$target.'>'.get_the_title().'</a></h2></div>
                <div class="post-images-item"><ul '.$style2.'>'.suxingme_get_thumbnail().'</ul></div><div class="posts-default-content"><div class="posts-text">'.$excerpt.'</div>
...
function fa_make_video_section(){
    global $post;
    ...
    if( true ){
        ...
        if( suxingme('suxingme_timthumb') && suxingme('suxingme_timthumb_lazyload',true) ) {
           // $image ='<img class="lazy thumbnail" data-original="'. get_template_directory_uri() .'/timthumb.php?src='.post_thumbnail_src().'&h=173.98&w=231.98&zc=1" src="'.constant("THUMB_SMALL_DEFAULT").'" alt="'. get_the_title().'" />';
            $image = '<video controls="" preload="metadata" src="'.$post->guid.'" class="arve-video fitvidsignore"></video>';
        }
        $image = '<video style="position:absolute;width: 100%; height:100%; left:0;top:0; object-fit: fill" controls="" preload="metadata" src="'.$post->guid.'" class="arve-video fitvidsignore"></video>';
        /*
        if ( suxingme('suxingme_timthumb') && !suxingme('suxingme_timthumb_lazyload',true) ) {
...
function fa_load_postlist_callback(){
    ...
    $videoTab = !empty($_POST["videoTab"]) ? $_POST["videoTab"] : "false";
    $query_args =null;
    if($videoTab=="true")
    {
        $query_args = array(
    ...
    while ( $the_query->have_posts() ){
        $the_query->the_post();
        if($videoTab=="true")
            $postlist .= fa_make_video_section();
        else
