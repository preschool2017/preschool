...
jQuery(document).on("click", "#fa-loadmore", function($) {
    ...
    if (_self.hasClass('is-loading')) {
        ...
        _videoTab = jQuery('#id_videoTab');
        if(_videoTab.hasClass("current"))
        {
            _data['videoTab']="true";//add by kali@neobv.com
        }
...
{
	//if(jQuery('#id_isvideolist').val() == 'isvideolist') jQuery('#fa-loadmore').click();
	if(jQuery("#id-indexpage").length<=0)
		jQuery(".video-post").eq(0).trigger('click');
   // alert("click");
