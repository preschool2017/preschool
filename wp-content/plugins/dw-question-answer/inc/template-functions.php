...
function dwqa_require_field_submit_question(){
    ...
    <div class="question-register clearfix">
        <label for="user-email"><?php _e('如果您还没有本站账号，那就请您动动小手注册一个吧！','dwqa') ?><a style="color:#a054a4;" href="http://www1.preschool.net.cn/%E6%96%B0%E7%94%A8%E6%88%B7%E6%B3%A8%E5%86%8C/">我要注册</a></label>
        ...
        <div class="register-username register-input">
            <!-- <input type="text" size="20" value="" class="input" placeholder="选择一个用户名" name="user-name-signup" id="user-name-signup"> -->
        </div>

        <div class="login-switch"><?php _e('难道您已经是本站用户了吗?','dwqa') ?> <a style="color:#a054a4;" href="http://www1.preschool.net.cn/login-registered/">我要登录</a></div>
...
function dwqa_require_field_submit_answer( $question_id ){
    ...
    <?php if( ! is_user_logged_in() ) { ?>
    <label for="answer_notify"><input type="checkbox" name="answer_notify" /> 新评论提醒</label>
...
function dwqa_submit_question_form(){
    ?>
    <div id="submit-question" class="dwqa-submit-question" style="width:85%;margin:0 auto;">    
        <?php  
