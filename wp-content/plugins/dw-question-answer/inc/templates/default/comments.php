...
?>
    ...
    <?php if ( get_comment_pages_count( get_the_ID() ) > 1 && get_option( 'page_comments' )  ) : ?>
    ...
    <?php if( ! dwqa_is_closed( get_the_ID() ) && dwqa_current_user_can( 'post_comment' ) ) { ?>
        <?php
            global $current_user;
            $args = array(
                'comment_field' => ((is_user_logged_in()) ? get_avatar( $current_user->ID, 32 ) : '') .'<textarea id="comment" name="comment" aria-required="true" placeholder="写一个答复"></textarea>',
